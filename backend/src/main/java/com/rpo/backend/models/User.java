...
// Отношение между таблицами cookers и restaurants многие-ко-многим, но так как между
// этими таблицами находится таблица meals у которой есть собственные поля, в
// дополнение к тем ключевым полям, которые связывают таблицы cookers и restaurants, то мы
...
public class User {     //вместо того, чтобы для каждого поля делать методы чтения и записи
    ...
    // addRestaurant и removeRestaurant - вспомогательные методы для addRestaurant и removeRestaurant в UserController
    // Решает проблему того, что для отношений многие-ко-многим изменения должны
    // быть сделаны сразу с двух сторон отношения.
    public void addRestaurant(Restaurant m) {
        this.restaurants.add(m);
        m.users.add(this);
    }

    public void removeRestaurant(Restaurant m) {
        this.restaurants.remove(m);
        m.users.remove(this);
    }
}
