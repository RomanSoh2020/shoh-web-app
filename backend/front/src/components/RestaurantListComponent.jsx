...
class RestaurantListComponent extends Component {
    ...
    deleteItemsClicked() {
        ...
        if (x.length > 0) {
            ...
            if (x.length > 1)
            {
                msg = "Пожалуйста подтвердите удаление " + x.length + " ресторанов";
            }
            else
            {
                msg = "Пожалуйста подтвердите удаление ресторана " + x[0].name;
            }
    ...
    refreshItems(cp) {
        BackendService.retrieveAllRestaurants(cp, this.state.limit)
            .then(
                resp => {
    ...
    onDelete()
    {
        BackendService.deleteRestaurants(this.state.selected_items)
