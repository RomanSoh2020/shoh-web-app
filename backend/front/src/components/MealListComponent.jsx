...
class MealListComponent extends Component {
    ...
    deleteItemsClicked() {
        ...
        if (x.length > 0) {
            ...
            if (x.length > 1)
            {
                msg = "Пожалуйста подтвердите удаление " + x.length + " блюд";
            }
            else
            {
                msg = "Пожалуйста подтвердите удаление блюда " + x[0].name;
            }
    ...
    refreshItems(cp) {
        BackendService.retrieveAllMeals(cp, this.state.limit)
            .then(
                resp => {
                    console.log("pts", resp.data.content);
    ...
    onDelete()
    {
        BackendService.deleteMeals(this.state.selected_items)
                    ...
                    <table className="table table-sm">
                        ...
                        <tbody>
                        {
                            this.state.items && this.state.items.map((item, index) =>
                                <tr key={item.id}>
                                    <td>{item.name}</td>
                                    <td>{item.cookers.name}</td>
                                    <td>{item.rest.name}</td>
                                    <td>
